...
from .point_conv import index_points_gather as index_points, index_points_group, Conv1d, square_distance ,KnnLoss, BallQLoss
...
class SmoothnessLoss(nn.Module):
    def __init__(self):
        ...
        self.knn_loss = KnnLoss(k=16, radius=0.25, loss_norm=1)
        self.ball_q_loss = BallQLoss(k=32, radius=0.75, loss_norm=1)
        self.w_knn = 0.7
        self.w_ball_q = 0.3
...
smooth_loss = SmoothnessLoss.apply
...
def multiScaleChamferSmoothCurvature(cur_pc1, cur_pc2, cur_flow):
    ...
    #smoothness
    #smoothness_value = computeSmooth(cur_pc1, cur_flow).sum(dim = 1).mean()
    #smoothnessLoss = smooth_loss(cur_pc1, cur_flow)
    ...
    #print("curvatureLoss:",curvatureLoss)

    return chamferLoss, curvatureLoss, 1
...
def Wassertein_loss(cur_pc1, cur_pc2, cur_flow):

    bs, _, _ = cur_pc1.shape
    cur_pc1_warp = cur_pc1 + cur_flow

    pc1_warp = cur_pc1_warp.permute(0,2,1).contiguous()
    pc2 = cur_pc2.permute(0,2,1).contiguous()
    
    L = SamplesLoss(loss='sinkhorn', debias=True, p=2, blur=0.01, scaling=0.99, backend="tensorized")
    #L = SamplesLoss(loss = 'gaussian', p=2, blur=.00005)
    
    ws_loss = L(pc1_warp, pc2)

    print(ws_loss.shape)
    mean_ws_loss = ws_loss.mean()

    #batch_WS = 0
    #for i in range(bs):
    #    ws_loss = L(pc1_warp[i], pc2[i])
    #    batch_WS += ws_loss
    
    #mean_ws_loss = batch_WS/bs

    return mean_ws_loss



def SSIM_loss(image1, image2, noc_mask=None, max_distance=1):
    """
    ...
    if noc_mask is not None:
        image1 = image1 * noc_mask
        image2 = image2 * noc_mask

    ssim_loss = 1 - ssim(image1, image2)
    loss = torch.clamp(ssim_loss.reshape(b,3,-1), min=0.0, max=1.0)
