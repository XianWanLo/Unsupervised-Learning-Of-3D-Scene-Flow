...
import transforms
from models.HALFlow import multiScaleChamferSmoothCurvature
from utils.flow_process import create_output
from evaluate import eval_kitti
...
def testOnce(scene_net, test_loader):
    ...
    for i, (pos1, pos2, color1, color2, flow, mask) in enumerate(test_loader):
        ...
        if i == 10:
            np.savetxt('./../cloud/finals/pc1_eval_2048.txt',(pos1).permute(0, 2, 1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc1_warped_eval_2048.txt',(pos1.permute(0, 2, 1)+flow).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc2_eval_2048.txt',(pos2).permute(0, 2, 1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f') 
        ...
        epe_ori.update(EPE3D_ori, args.test_batch_size)
        acc1_ori.update(acc3d_strict_ori, args.test_batch_size)
        acc2_ori.update(acc3d_relax_ori, args.test_batch_size)
        outliers.update(outlier, args.test_batch_size)

        #flow_pred_2d, flow_gt_2d = get_2d_flow(pos1, pos1 + tar, pos1 + pred_sf)
        #EPE2D_ori, acc2d_ori = evaluate_2d(flow_pred_2d, flow_gt_2d)
        #epe_2d.update(EPE2D_ori, args.test_batch_size)
        #acc1_2d.update(acc2d_ori, args.test_batch_size)
...
def main():
    ...
    with open(log_dir + '/log.csv', 'w') as csvfile:
        ...
        writer.writerow(['train_loss  ', 'eva_epe  ', 'eva_acc1  ', 'eva_acc2  ', 'outlier  ', 'epe2d  ','acc2d' ])
    ...
    if args.evaluate:
        ...
        print("epe3d without mask: ", epe)
        print("acc3d strict without mask: ", acc1)
        print("acc3d relax without mask: ", acc2)
        #print("epe2d without mask: ", epe2d)
       # print("acc2d without mask: ", acc2d)
        ...
        for epoch in range(args.epochs):
            ...
            train_loader = torch.utils.data.DataLoader(train_part_dataset, batch_size=args.batch_size, shuffle=True,
                                                        num_workers=args.workers, pin_memory=True)
            ...
            if epoch % args.test_period == 0:
                ...
                #training_writer.add_scalar("eped2d_ori:", epe2d, epoch)
                #training_writer.add_scalar("acc2d_ori", acc2d, epoch)
...
def train(train_loader, disp_net, pose_net, scene_net, optimizer, training_writer=None):
    ...
    for i_data, (tgt_img, ref_imgs, intrinsics, inv_intrinsics, velo2cam, cam2velo, ori_tgt_img, ori_ref_imgs, depth_gt) in enumerate(train_loader):

        tgt_img = tgt_img.to(device)  # [b 3 h w] #TODO目标帧
        ref_imgs = [img.to(device) for img in ref_imgs]  #  [b 3 h w] #TODO 下一帧1和上一帧0
        intrinsic = intrinsics.to(device)  # [b 3 3] #TODO内参数矩阵
        inv_intrinsic = inv_intrinsics.to(device)  # [b 3 3]#TODO内参数矩阵的逆矩阵
        ...
        depth = depth[0] #TODO target帧的深度图

        fw_depth = ref_depths[1][0] #TODO 下一帧的深度图
        bw_depth = ref_depths[0][0] #TODO 上一帧的深度图

        pose_predict = pose_net(tgt_img, ref_imgs[1])
        pose_mat = pose_vec2mat(pose_predict) # TODO [B, 3, 4]
        ...
        side_mask =  block_mask1 * block_mask2
        ...
        if total_mask_sum.sum().item() > 0:#TODO 限定每个batch中有效点个数>index_num
            continue
        else:
            for i in range(b):
                f = (total_mask_flatten[i, :] > 0).nonzero(as_tuple=False)
                f = f.view(-1)
                n_f = f.cpu().numpy()
                np.random.shuffle(n_f)#TODO 一个batch中所有不为0的元素索引
        ...
        if i_data == 30:
            ...
            total_mask_b1n = total_mask_flatten.unsqueeze(1)
            side_mask_b1n = side_mask.view(b, -1).unsqueeze(1)
            
            pc1_ori = pc1_cam1_flatten.permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            color1_ori = (ori_rgb1_flatten*255).permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            create_output(pc1_ori, color1_ori, './../cloud/finals/pc1_ori_color.ply')

            pc1_ori_side_masked = (pc1_cam1_flatten*side_mask_b1n).permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            color1_ori_side_masked = (ori_rgb1_flatten*255*side_mask_b1n).permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            create_output(pc1_ori_side_masked, color1_ori_side_masked, './../cloud/finals/pc1_ori_block_masked.ply')

            pc1_ori_masked = (pc1_cam1_flatten*total_mask_b1n).permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            color1_ori_masked = (ori_rgb1_flatten*255*total_mask_b1n).permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            create_output(pc1_ori_masked, color1_ori_masked, './../cloud/finals/pc1_ori_masked.ply')

            pc1_ori_8192 = pc1_cam1_part.permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            color1_ori_8192 = (ori_rgb1_part*255).permute(0,2,1).reshape(-1,3).cpu().detach().numpy()
            create_output(pc1_ori_8192, color1_ori_8192, './../cloud/finals/pc1_ori_8192.ply')

            np.savetxt('./../cloud/finals/pc1_ori.txt',(pc1_cam1_flatten).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc2_ori.txt',(pc2_cam2_flatten).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')

            np.savetxt('./../cloud/finals/pc1_ori_masked.txt',(pc1_cam1_flatten*total_mask_b1n).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc2_ori_masked.txt',(pc2_cam2_flatten*total_mask_b1n).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
    
            np.savetxt('./../cloud/finals/pc1_ori_8192.txt',(pc1_cam1_part).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc2_ori_8192.txt',(pc2_cam2_part).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
        ...
        if i_data == 30:
            ...
            np.savetxt('./../cloud/finals/pc1_trained_2048.txt',(pc1_cam1_part[:, :, idx1s[0][0]]).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc1_warped_trained_2048.txt',((pc1_cam1_part[:, :, idx1s[0][0]]+over_all_flows[0])).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/finals/pc2_trained_2048.txt',(pc2_cam2_part[:, :, idx1s[0][0]]).permute(0,2,1).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')  
        ...
        add_3 = torch.from_numpy(add_3).type_as(pose_mat)

        random_R = R_trans.unsqueeze(0).repeat(b, 1, 1)  # [b 3 3]
        random_T = add_3.unsqueeze(0).repeat(b, 1, 1)  # [b 3 1]
        pose_R = pose_mat[:, :, :3]  # [b 3 3]
        pose_T = pose_mat[:, :, 3:]  # [b 3 1]
        new_R = pose_R.bmm(random_R)
        new_T = pose_R.bmm(random_T) + pose_T
        ...
        for level_id, overall_sf in enumerate(over_all_flows):
            ...
            if level_id == 0:
                ...
                pcoords = intrinsic.bmm(sf_pc2)  # [b 3 n]#TODO摄像机2下的像素坐标（齐次）* 深度 sf_pc2:估计的PC_2^2
                X = pcoords[:, 0]
                Y = pcoords[:, 1]
                ...
                grid_x = Variable(torch.arange(0, w).view(1, 1, w).expand(1, h, w), requires_grad=False).type_as(X_norm).expand(b, h, w)  # [bs, H, W] #TODO b * h * [0,1,...,w-1]
                grid_y = Variable(torch.arange(0, h).view(1, h, 1).expand(1, h, w), requires_grad=False).type_as(Y_norm).expand(b, h, w)  # [bs, H, W] #TODO b *  [0,1,...,h-1]^T * w
                grid_x = 2 * (grid_x / (w - 1.0) - 0.5)#TODO宽度转换到[-1，1]的区间
                grid_y = 2 * (grid_y / (h - 1.0) - 0.5)#TODO高度转换到[-1，1]的区间

                grid_x_flat = grid_x.view(b, -1)#TODO b * h * [0,1,...,w-1] [b,n] 转换到[-1,1]
                grid_y_flat = grid_y.view(b, -1)#TODO b * [0*w,1*w,...,h-1*w] [b,n]转换到[-1,1]

                grid_x_flat = set_by_index(grid_x_flat, X_norm, index1[:, idx1[0]])#TODO 被选中的有效点中提取出来的该层用的点的索引
                grid_y_flat = set_by_index(grid_y_flat, Y_norm, index1[:, idx1[0]])
                grid_x = grid_x_flat.view(b, h, w)
                grid_y = grid_y_flat.view(b, h, w)

                grid_tf = torch.stack((grid_x, grid_y), dim=3)#TODO [b,h,w,2] x,y一一对应

                pro_depth = torch.nn.functional.grid_sample(new_fw_depth, grid_tf, padding_mode='border')
                pro_xyz = BackprojectDepth(pro_depth, inv_intrinsic)#TODO 输入估计的深度和内参数矩阵逆阵，返回估计点云（带hat的）
                pro_xyz_flatten = pro_xyz.view(b, 3, -1)
                pro_part_xyz = devide_by_index(pro_xyz_flatten, index1[:, idx1[0]])

                pro_img = torch.nn.functional.grid_sample(ref_imgs[1], grid_tf, padding_mode='border')
                pro_img_flatten = pro_img.view(b, 3, -1)
                pro_part_img = devide_by_index(pro_img_flatten, index1[:, idx1[0]])
                
                tag_part_img = devide_by_index(rgb1_flatten, index1[:, idx1[0]])

                loss4 = (pro_part_xyz - sf_pc2).abs().mean() * weight_list[level_id]
                loss9 = (pro_part_img - tag_part_img).abs().mean() * weight_list[level_id]
                ...
                #loss91 = (pro_part_img - tag_part_img).abs().mean() * weight_list[level_id]
        ...
        training_writer.add_scalar("loss4_Depth_Consistency_Loss", loss4, n_iter)
        training_writer.add_scalar("loss5_overall_static_dynamic_Loss", loss5, n_iter)
        training_writer.add_scalar("loss7_chamfer_loss", loss7, n_iter)
        training_writer.add_scalar("loss8_curvature_loss", loss8, n_iter)
        training_writer.add_scalar("loss9_photometric_loss", loss9, n_iter)

        training_writer.add_scalar("loss41_Depth_Consistency_Loss_of_new_flow", loss41, n_iter)
        #training_writer.add_scalar("loss91_Photometric_Loss_of_new_flow", loss91, n_iter)
        loss = args.loss_weight_dc * (loss4 + loss41) + args.loss_weight_ods * loss5 + loss7 * args.chamfer_loss_weight + loss8 * args.curvature_loss_weight + args.photometric_loss_weight * (loss9) 
