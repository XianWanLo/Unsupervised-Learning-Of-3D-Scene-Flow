...
import transforms
from models.HALFlow import multiScaleChamferSmoothCurvature,SSIM_loss
...
def testOnce(scene_net, test_loader):
    ...
    for i, (pos1, pos2, color1, color2, flow, mask) in enumerate(test_loader):
        ...
        #print("EPE3D:",EPE3D_ori)
        #print("acc3d_strict:",acc3d_strict_ori)
        #print("acc3d_relax:",acc3d_relax_ori)
...
def train(train_loader, disp_net, pose_net, scene_net, optimizer, training_writer=None):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, inv_intrinsics, velo2cam,cam2velo, ori_tgt_img, ori_ref_imgs, depth_gt) in enumerate(train_loader):
        ...
        #weight_list = [0.2, 0.1, 0.05, 0.025]
        ...
        for level_id, overall_sf in enumerate(over_all_flows):
            ...
            #! grid_x_flat[i,...,index1[:, idx1[0]] = X_norm[i]
            ...
            ssim_loss = SSIM_loss(tgt_img, pro_img, noc_mask=total_mask.unsqueeze(1), max_distance=1)
            ssim_valid = devide_by_index(ssim_loss, index1[:, idx1[0]])
            
            if level_id == 0:
                loss4 = (pro_part_xyz - sf_pc2).abs().mean() * weight_list[level_id]
                loss9 = ssim_valid.mean() * weight_list[level_id]
                print("SSIM loss:",loss9)
        ...
        training_writer.add_scalar("loss9_SSIM_loss", loss9, n_iter)

        training_writer.add_scalar("loss41_Depth_Consistency_Loss_of_new_flow", loss41, n_iter)
        loss = args.loss_weight_dc * (loss4 + loss41) + args.loss_weight_ods * loss5 + loss7 * args.chamfer_loss_weight + loss8 * args.curvature_loss_weight + 0.1*loss9
