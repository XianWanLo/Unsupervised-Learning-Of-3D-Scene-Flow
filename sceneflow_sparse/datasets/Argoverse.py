...
class ArgoverseSceneFlowDataset(Dataset):
    def __init__(self, root, partition="val", width=1):
        ...
        if self.partition == "val":
            self.datapath = sorted(glob.glob(f"{self.val_path}/val/*/*"))
    ...
    def __getitem__(self, index):
        ...
        color1 = np.zeros([num_points, 3])
        color2 = np.zeros([num_points, 3])
