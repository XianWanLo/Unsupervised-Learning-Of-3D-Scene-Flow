...
from model.model_full import ESBFlow,get_selected_idx
from model.point_conv_pytorch import warppingProject,fast_select
...
from evaluate import eval_KITTI_train
from sparse_loss import Dense_Chamfer_Curvature
...
def main():
    ...
    log_dir = args.save_path + '/' + 'train_try_' + today
    ...
    with open(log_dir + '/train_loss.csv', 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow([ 'training loss '])

    torch.manual_seed(args.seed)
    os.system('cp %s %s' % ('train_try.py', log_dir))
    ...
    if args.evaluate:  #只需要evaluate
        ...
        epe, acc1, acc2,outlier, epe2d , acc2d = eval_KITTI_train(scene_net, test_loader,disp_net,pose_net)
        ...
        #print("Evaluate once before training:")
        #EVALUATE
        #epe3d_pre2, acc1_pre2, acc2_pre2,outlier_pre2, epe2d , acc2d = eval_kitti_sceneflow(scene_net, test_loader)

        #print("original epe3d: ", epe3d_pre2)
        #print("original acc3d strict: ", acc1_pre2)
        #print("original acc3d relax: ", acc2_pre2)
        #print("original outlier3d: ", outlier_pre2)
        #print("original epe2d: ", epe2d)
        #print("original acc2d strict: ", acc2d)
        ...
        for epoch in range(args.epochs):
            ...
            evaluate_acc = acc1_pre2 + acc2_pre2 + acc2d

            if evaluate_acc > best_acc:
                best_acc = evaluate_acc
                torch.save({'epoch': epoch + 1, 'state_dict': scene_net.state_dict()}, log_dir + '/scene_model.best.t7')
...
def train(train_loader, disp_net, pose_net, scene_net, optimizer, training_writer=None):
    ...
    for i_data, (tgt_img, ref_imgs, intrinsics, inv_intrinsics, velo2cam,cam2velo, ori_tgt_img, ori_ref_imgs, depth_gt) in enumerate(train_loader):
        ...
        outliers_mask = build_outliers_mask(pc1_cam1,pc2_cam2)
        ...
        total_mask = sky_mask1 * sky_mask2 * ground_mask1 * ground_mask2 * block_mask1 * block_mask2  *outliers_mask * angle_sky_mask.type_as(ground_mask2)
        ...
        not_valid_batch = (total_mask_sum < 5000)

        if not_valid_batch.sum().item() > 0:
            print("continue")
            continue

        total_mask_bhw1 = (total_mask.unsqueeze(-1)).contiguous()                 # [B H W 1]
        #ground_mask =  (ground_mask1 * ground_mask2).unsqueeze(-1).contiguous() 
        ...
        if i_data == 0:
            np.savetxt('./../cloud/pc1_ori.txt',(pc1_cam1_bhw3[0]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/pc2_ori.txt',(pc2_cam2_bhw3[0]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
        ...
        if i_data == 0:
            np.savetxt('./../cloud/pc1_trained.txt',(pc1[0][0,:,:])[mask_list[0][0,:] > 0].reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/pc1_warped_trained.txt',((pc1[0][0,:,:]+over_all_flows[0][0,:,:])[mask_list[0][0,:] > 0]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/pc2_trained.txt',(pc2[0][0,:,:])[mask_list[0][0,:] > 0].reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')  

            np.savetxt('./../cloud/pc1_trained_unmasked.txt',(pc1[0][0,:,:]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/pc1_warped_trained_unmasked.txt',((pc1[0][0,:,:]+over_all_flows[0][0,:,:])).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            np.savetxt('./../cloud/pc2_trained_unmasked.txt',(pc2[0][0,:,:]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')  
        ...
        for level_id, overall_sf in enumerate(over_all_flows):
            ...
            for i in range(b):
                valid_num = valid[i,:].nonzero().view(-1).shape[-1]
                if valid_num < min_num:
                    min_num = valid_num

            if min_num < 30: continue

            index1 = np.zeros((b,min_num))
            index2 = np.zeros((b,min_num))
            ...
            for i in range(b):
                ...
                index1[i, :] = n_f[:min_num]
                #np.random.shuffle(n_f)
                index2[i, :] = n_f[:min_num]

            print("batch valid:",index1.shape)
            ...
            if level_id == 0:
                ...
                #warped_xyz_indexed = devide_by_index(warped_pc1_b3n, index1)
                
                #^ 相机三维点云 [X,Y,Z] 投影到 相机二维像素阵列 [u,v] = [Fx*X/Z+Cx, Fy*Y/Z+Cy] 
                pcoords = intrinsic.bmm(warped_pc1_b3n)      #! pc1坐标上的pc1_warped
                #pcoords = new_intr.bmm(warped_pc1_b3n)
                X_ = pcoords[:, 0]
                Y_ = pcoords[:, 1]
                Z = pcoords[:, 2].clamp(min=1e-3)
                
                #^ 二维像素阵列 分别除 w 和 h ,得到归一化像素平面 [0-1, 0-1]
                U_norm = (2*(X_ / Z) / (WIDTH - 1) - 1)      # Normalized, -1 if on extreme left, 1 if on extreme right (x = w-1) [B, H*W]
                V_norm = (2*(Y_ / Z) / (HEIGHT - 1) - 1)

                #? 3/30新加的
                grid_x = Variable(torch.arange(0, WIDTH).view(1, 1, WIDTH).expand(1, HEIGHT, WIDTH), requires_grad=False).type_as(U_norm).expand(b, HEIGHT, WIDTH)  # [bs, H, W]
                grid_y = Variable(torch.arange(0, HEIGHT).view(1, HEIGHT, 1).expand(1, HEIGHT, WIDTH), requires_grad=False).type_as(V_norm).expand(b, HEIGHT, WIDTH)  # [bs, H, W]
                grid_x_full = 2 * (grid_x / (WIDTH - 1.0) - 0.5)
                grid_y_full = 2 * (grid_y / (HEIGHT - 1.0) - 0.5)
                
                grid_x = torch.where(valid,U_norm,grid_x_full)
                grid_y = torch.where(valid,V_norm,grid_y_full)
                
                grid_tf = torch.stack([grid_x, grid_y], dim=3).cuda()
                  
                #^ depth consistency   
                pro_depth = torch.nn.functional.grid_sample(new_fw_depth, grid_tf, padding_mode='border',align_corners=True)
                
                #^ 深度[z] 加上 二维像素阵列 [u,v]  ， 乘以逆内参后得到 三维点云 [x,y,z]
                pro_xyz = BackprojectDepth(pro_depth, inv_intrinsic)        # 三维点云 = 逆参矩阵 * 像素深度     [B 3 H W]
                pro_xyz_flatten = pro_xyz.view(b, 3, -1)
                
                if i_data == 20:
                    np.savetxt('./../cloud/warped_xyz.txt',((warped_pc1[0,:,:])[valid[0,:]]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
                    np.savetxt('./../cloud/pro_xyz.txt',(((pro_xyz_flatten.permute(0,2,1))[0,:,:])[valid[0,:]]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
    
                depth_con1_n3 = (abs(pro_xyz_flatten - warped_pc1_b3n).permute(0,2,1))[valid]
                ...
                #valid_b3n = valid.unsqueeze(1).repeat(1,3,1)
                #depth_con1_n3 = abs(pro_xyz_flatten - warped_pc1_b3n) * valid_b3n      # [B 3 N]
                #depth_con1_loss = (depth_con1_n3.sum() / valid_b3n.sum()) * weight_list[level_id] 
                #depth_loss = depth_con1_loss  
                
                #if level_id ==0:
                #    np.savetxt('./../cloud/pro_xyz_1.txt',(pro_xyz_flatten.permute(0,2,1)[valid]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
                #    np.savetxt('./../cloud/warped_pc1.txt',(warped_pc1[valid]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
                ...
                #if level_id ==0:
                    #[mask_list[0]>0]
                #    np.savetxt('./../cloud/pro_xyz_2.txt',(pro_xyz_flatten.permute(0,2,1)[valid]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
                #    np.savetxt('./../cloud/new_warped_pc1.txt',(new_warped_pc1[valid]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            ...
            #print("pc2[level_id]:",pc2[level_id].shape)
            #np.savetxt('./../cloud/pc2[level_id].txt',(pc2[level_id]).reshape(-1,3).cpu().detach().numpy(),fmt='%f %f %f')
            
            chamfer_loss, curvature_loss = multiScaleChamferSmoothCurvature(pc1[level_id], pc2[level_id], overall_sf, new_intr, h ,w , valid)
            valid_num = mask_list[0].reshape(b,-1).sum()          # sum of valid number across all batches
            #print("mean valid points for each level:",mask_list[level_id].reshape(b,-1).sum(dim=-1).mean())
            
            #chamfer_loss = (chamfer_loss / valid_num)* weight_list[level_id]
            #curvature_loss = (curvature_loss / valid_num) * weight_list[level_id]
            
            npoints = index1.shape[-1]

            #chamfer_loss, curvature_loss = Dense_Chamfer_Curvature(pc1[level_id], pc2[level_id], overall_sf, valid , index1, index2)
            #chamfer_loss = (chamfer_loss / (256*832)) * weight_list[level_id]
            #curvature_loss = (curvature_loss / (256*832)) * weight_list[level_id]
        ...
        print("overall depth consistency loss1:",depth_loss)
        ...
        loss =  args.loss_weight_dc * depth_loss + args.chamfer_loss_weight * loss7 + args.curvature_loss_weight * loss8
...
def eval_kitti_sceneflow(scene_net, test_loader):
    ...
    for i, data in enumerate(test_loader, 0):
        ...
        flow3d_list, mask_list, pc1_sample ,pc2_sample ,height_list, width_list, flow3d_gt_list = model(pc1, pc2, color1, color2, intrinsics, occ_mask.unsqueeze(-1), flow, False, False)
        ...
        flow_pred_2d, flow_gt_2d = get_batch_2d_flow(pc1_np, pc1_np + sf_np, pc1_np + pred_sf, intrinsic_np, height_list[0], width_list[0], HEIGHT, WIDTH)
...
def eval_kitti_odometry(scene_net, test_loader):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, inv_intrinsics, velo2cam,cam2velo, ori_tgt_img, ori_ref_imgs, depth_gt) in enumerate(train_loader):
        ...
        new_pred_depth = depth
        new_fw_depth = fw_depth

        pc1 = BackprojectDepth(new_pred_depth, inv_intrinsic)
        pc2 = BackprojectDepth(new_fw_depth, inv_intrinsic)
