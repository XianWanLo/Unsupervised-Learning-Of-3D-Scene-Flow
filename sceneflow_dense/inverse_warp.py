...
def coor_trans(ori_coor, pose_mat):
    ...
    rot_mat = rot_mat.unsqueeze(0).repeat(b,1,1)
    trans_mat = trans_mat.unsqueeze(0).repeat(b,1,1)
...
def build_angle_sky(pc, velo2cam, cam2velo):
    ...
    mask = arc_coor<0.035                   #^ 从 0.035 - 0.05
    mask = mask + 0.0              # [b  h*w]
...
def build_sky_mask(depth):
    ...
    for i in range(b):
        ...
        valid_depth = temp_depth<60 #很远（深度）       #^  3/17: 35 --> 60
...
def build_groud_mask(pc):
    ...
    for i in range(b):
        ...
        valid_depth = temp_pc_y< 1.4           #^  3/17: 1.15 --> 61.4
...
def BackprojectDepth(depth, inv_k):
    ...
    pix_coords = torch.unsqueeze(torch.stack([id_coords[0].view(-1), id_coords[1].view(-1)], 0), 0)  #1 h w 
...
def build_block_mask(tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic, inv_intrinsic):
    """ 
        return [b h w]
    """ 
    #参考Unsupervised Scale-Consistency论文的Geometry consistency loss（pg5) 
    ### WARP
