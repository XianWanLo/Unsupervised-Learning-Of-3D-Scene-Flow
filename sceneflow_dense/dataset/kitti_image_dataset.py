...
import random
import dataset.dataset_utils.custom_transforms as custom_transforms
import torch
...
def crawl_folders(folders_list, sequence_length):
    ...
    for folder in folders_list:
        ...
        for i in range(demi_length, len(imgs) - demi_length):
            sample = {'intrinsics': intrinsics, 'tgt': imgs[i], 'ref_imgs': [], 'ground_truth': gts[i]}
...
class SequenceFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        tgt_img = load_as_float(sample['tgt'])      # W H C
        ref_imgs = [load_as_float(ref_img) for ref_img in sample['ref_imgs']]

        depth_gt = np.load(sample['ground_truth'])

        if self.transform is not None:
            imgs, intrinsics = self.transform([tgt_img] + ref_imgs, np.copy(sample['intrinsics'])) #  C H W
        ...
        return tgt_img, ref_imgs, intrinsics, np.linalg.inv(intrinsics), tgt_img_1, ref_imgs_1, depth_gt
...
if __name__ == '__main__':
    ...
    for i, (tgt_img, ref_imgs, intrinsics, inv_intrinsics, ori_tgt_image, ori_ref_imgs, depth_gt) in enumerate(train_loader):
        ...
        #print("inv_intrinsics", inv_intrinsics)
