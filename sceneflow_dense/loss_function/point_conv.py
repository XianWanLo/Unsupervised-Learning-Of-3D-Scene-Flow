...
from Pointnet2 import pointnet2_utils_pytorch
...
def index_points_gather(points, fps_idx):
    ...
    Input:
        ...
        fps_idx: sample index data, [B, S]
    Return:
        new_points:, indexed points data, [B, S, C]
    """

    points_flipped = points.permute(0, 2, 1).contiguous()
    new_points = pointnet2_utils_pytorch.gather_operation(points_flipped, fps_idx)
...
def index_points_group(points, knn_idx):
    ...
    """
    points_flipped = points.permute(0, 2, 1).contiguous()
    new_points = pointnet2_utils_pytorch.grouping_operation(points_flipped, knn_idx.int()).permute(0, 2, 3, 1)
...
def sample_and_group(flag_big_radius, kernel_shape, layers, npoint, radius, nsample, xyz, xyz_raw, label, points, knn=True, use_xyz=True):
    '''
    Input:
        npoint: int32
        radius: float32
        nsample: int32
        xyz: (batch_size, ndataset, 3) TF tensor  channel
    ...
    b = list(xyz.shape)[0]
    if npoint > 0:
        new_xyz_list = torch.split(xyz, npoint, 1)  # (batch_size, npoint, 3)
        new_label_list = torch.split(label, npoint, 1)   # (batch_size, npoint, 3)
        sample_idx = ((torch.arange(0,npoint)).unsqueeze(dim=0)).repeat([b,1])
        new_xyz = new_xyz_list[0]
        new_label = new_label_list[0]
        if xyz_raw is not None:
            new_xyz_raw_list = torch.split(xyz_raw, npoint, 1)  # (batch_size, npoint, 3)
            new_xyz_raw = new_xyz_raw_list[0]
    else:
        sample_idx = pointnet2_utils_pytorch.furthest_point_sample(xyz, npoint)  # (batch_size,npoint)
    ...
    if xyz_raw is not None:
        return new_xyz, new_label, new_points, sample_idx, new_xyz_raw
    else:
        return new_xyz, new_label, new_points, sample_idx  #(batch_size, npoint, 3) (batch_size, npoint, 3) (batch_size, npoint, nample, 3+c) (batch_size,npoint)
...
class PointNetSaModule(nn.Module):
    def __init__(self, flag_big_radius, kernel_shape, layers, npoint, radius, nsample, in_channels,mlp, mlp2, group_all, is_training, bn_decay, bn=True, pooling='max', knn=False, use_xyz=True):
        super(PointNetSaModule, self).__init__()
        self.flag_big_radius = flag_big_radius
    ...
    def forward(self, xyz, xyz_raw, label, points ):
        ...
        if xyz_raw is not None:
            # [B,npoint,3] [B,npoint,3] [B,npoint,nsample,3+C] [B,npoint] [B,npoint,3]
            new_xyz, new_label, new_points, sample_idx, new_xyz_raw = sample_and_group(self.flag_big_radius, self.kernel_shape, self.layers, self.npoint, self.radius,self.nsample, xyz, xyz_raw, label, points, self.knn, self.use_xyz)
        else:
            new_xyz, new_label, new_points,sample_idx = sample_and_group(self.flag_big_radius, self.kernel_shape, self.layers, self.npoint, self.radius,self.nsample, xyz, xyz_raw, label, points, self.knn, self.use_xyz)
        ...
        if xyz_raw is not None:
            return new_xyz, new_label,  new_points,sample_idx.numpy(), new_xyz_raw
        else:
            return new_xyz, new_label,  new_points, sample_idx.numpy()

class CostVolume(nn.Module):
    def __init__(self,flag_big_radius, kernel_shape, layers,radius, nsample, nsample_q,in_channels,mlp1, mlp2, is_training, bn_decay,bn=True, pooling='max', knn=True, corr_func='elementwise_product'):
...
class SetUpconvModule(nn.Module):
    def __init__(self,flag_big_radius, kernel_shape, layers, nsample, in_channels,mlp, mlp2, is_training, bn_decay=None, bn=True, pooling='max', radius=None, knn=True):
...
class PointnetFpModule(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        dist, idx = pointnet2_utils_pytorch.three_nn(xyz1, xyz2)  #(b,n1,3)
        dist[dist < 1e-10] = 1e-10
        norm = torch.sum((1.0 / dist), dim=2, keepdim=True)
        norm = norm.repeat(1,1,3)
        weight = (1.0 / dist) / norm
        points2 = points2.permute(0,2,1)
        interpolated_points = pointnet2_utils_pytorch.three_interpolate(points2, idx, weight)
...
class PointConvD(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        fps_idx = pointnet2_utils_pytorch.furthest_point_sample(xyz, self.npoint)
