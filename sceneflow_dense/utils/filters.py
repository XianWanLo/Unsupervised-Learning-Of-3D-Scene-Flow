...
def depth2ptc(depth, calib):
    ...
    points = points.t()
    return calib.img_to_lidar(points[:, 0], points[:, 1], points[:, 2])
...
def GDC(pred_depth, gt_depth, calib,
    ...
    consider_L = filter_mask(depth2ptc(gt_depth, calib).cpu().numpy()
                             ).reshape(gt_depth.shape)
    consider_L = consider_L
    ...
    #print('pred_depth[pred_mask]',pred_depth[pred_mask],"pred_depth[gt_mask]",pred_depth[gt_mask],gt_mask)
...
class FliterGDC(object):
    def __init__(self, pred_depth):
        self.pred_depth = pred_depth
        
    def DepthCorrection(self, pred_depth, gt_depth, calib):
        new_depth_map = GDC(pred_depth, gt_depth, calib,k=10,W_tol=1e-5,recon_tol=1e-4,verbose=False,method='gmres',consider_range=(-0.1, 3.0),subsample=False)
        return new_depth_map
...
class FilterR(object):
    ...
    def fliter_radius(self, pc_data):
                
        pcl = open3d.geometry.PointCloud()
        pcl.points = open3d.utility.Vector3dVector(self.pc_data)
        #print(pcl)
        #cl, ind = pcl.remove_radius_outlier(nb_points=30, radius=0.5)
        cl, ind = pcl.remove_statistical_outlier(nb_neighbors=8, std_ratio=2.0)
        #print(cl)
        cl_arr=np.asarray(cl.points)
        #pc_data = np.delete(self.pc_data,-1,axis=1)
        #np.set_printoptions(suppress=True)
        '''pcs = []
        for i in range(len(self.pc_data)):
            datas = self.pc_data[i]
            print(datas)
            pcl = open3d.geometry.PointCloud()
            pcl.points = open3d.utility.Vector3dVector(datas)
            cl, ind = pcl.remove_radius_outlier(nb_points=20, radius=0.2)
            cl_arr=np.asarray(cl.points)
            pc = torch.tensor(cl_arr, dtype=torch.float32)
            pcs.append(pc)
            '''
        return cl_arr, ind