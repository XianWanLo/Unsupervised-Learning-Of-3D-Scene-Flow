...
def build_groud_mask(pc,ground_limit):
    ''' 
        input: 
            pc [b 3 h w]
        output: 
    ...
    for i in range(b):
        ...
        valid_depth = temp_pc_y < ground_limit
...
def build_outliers_mask(pc1_b3hw,pc2_b3hw):

    b,_,h,w = pc1_b3hw.shape
    pc1 = pc1_b3hw.permute(0,2,3,1).reshape(b,-1,3).contiguous()
    pc2 = pc2_b3hw.permute(0,2,3,1).reshape(b,-1,3).contiguous()

    new_valid_mask_pc1 = np.zeros((b,h*w,1))
    new_valid_mask_pc2 = np.zeros((b,h*w,1))

    for batch in range(b):

        #^ 分批去除pc1离群点
        velo_points1 = pc1[batch,:].reshape(-1,3)
        pc1_data = velo_points1[:,:3].cpu().detach().numpy()
        data = FilterR(pc1_data)
        data1,index1 = data.fliter_radius(pc1_data)      # 返回哪些点不是离群点的索引
        #print("index1:",index1.size)
        #velo_points1 = velo_points1[index]              # 把非离群点索引出来

        #^ 分批去除pc2离群点
        velo_points2 = pc2[batch,:].reshape(-1,3)
        pc2_data = velo_points2[:,:3].cpu().detach().numpy()
        data = FilterR(pc2_data)
        data2,index2 = data.fliter_radius(pc2_data)      # 返回哪些点不是离群点的索引
        #velo_points2 = velo_points2[index]              # 把非离群点索引出来

        new_valid_mask_pc1[batch][index1] =  1.0 
        new_valid_mask_pc2[batch][index2] =  1.0 

    valid_mask = torch.from_numpy(new_valid_mask_pc1 * new_valid_mask_pc2).reshape(b,h,w).cuda()
